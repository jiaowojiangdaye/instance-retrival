...
def cosine_sim(a, b):
    ...
    a, b []
    '''
    length = int(a.get_shape()[-1])
    a = tf.reshape(a, [1, length])
    b = tf.reshape(b, [1, length])
    mult0 = tf.matmul(a, tf.transpose(b))
    mult1 = tf.matmul(a, tf.transpose(a))
    mult2 = tf.matmul(b, tf.transpose(b))
    

    sqrt = K.sqrt(mult1+mult2)
    cosin_smi = tf.divide(mult0, sqrt)
#    cosin_smi = tf.squeeze(cosin_smi)
    return cosin_smi

def o_dist(a, b):
    
    pos_dist = tf.reduce_sum(tf.square(tf.subtract(a, b)), 0)
    
    return pos_dist

def rmac_loss_graph(config, feats_norm_ori):
    """
    Return the RPN bounding box loss graph.
    feats_norm[0]: [batch, gt_num, 256]  in fact gt_num=1 always here
    config: the model config object.
    """
    vector_length = int(feats_norm_ori.get_shape()[1])
    positive, negative = tf.unstack(tf.reshape(feats_norm_ori, [-1,2,vector_length]), 2, 1)
    
    half_num = int(config.IMAGES_PER_GPU/2)
    positive_l = tf.unstack(positive, num=half_num, axis=0)
    negative_l = tf.unstack(negative, num=half_num, axis=0)
    cosin_loss_l = []
    for i in range(half_num):
        cosin_loss_l.append(cosine_sim(positive_l[i], negative_l[i]))
    
    cosin_loss_c = tf.concat(cosin_loss_l, axis=0)
    cosin_loss = tf.reduce_mean(cosin_loss_c, axis=0)#1
    cosin_loss = tf.squeeze(cosin_loss)
    
    return 1.0- cosin_loss

def triplet_loss_graph(config, feats_norm_ori):
    """
    Return the RPN bounding box loss graph.
    feats_norm[0]: [batch, gt_num, 256]  in fact gt_num=1 always here
    config: the model config object.
...
#    feats_norm = feats_norm[0]
#    embeddings = tf.squeeze(feats_norm_ori, axis=[1])    #[[batch, 256]]
...
#    embeddings = tf.nn.l2_normalize(x=embeddings, axis=1, name='embeddings')
    ...
    neg_dist = tf.reduce_sum(tf.square(tf.subtract(positive, negative)), 1)

    
    triplet_loss = tf.subtract(2.0, neg_dist)
...
def build_train_rmac_graph(rois, feature_maps, image_meta,
                         pool_size):
    """Builds the computation graph of the mask head of Feature Pyramid Network.
    ...
    # Shape: [batch, num_rois, MASK_POOL_SIZE, MASK_POOL_SIZE, channels]
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_rmac")([rois, image_meta] + feature_maps)
    #[batch, num_boxes, pool_height, pool_width, channels].
#    feats = KL.Maximum(axis=(2, 3), name='rmac_maxpooling')(x)
    
    feats = KL.Lambda(lambda x: K.max(x, axis=(2, 3)))(x)
    
    #[batch, num_boxes, maximum, channels].
#    x = KL.Lambda(lambda x: K.l2_normalize(x, axis=2), name='feats_norm')(x)
#    feats = KL.Lambda(lambda x: K.squeeze(x, 2),
#                       name="pool_squeeze")(feats)
    #[batch, num_boxes, channels].
    # Normalization
    feats_norm = KL.Lambda(lambda x: K.l2_normalize(x, axis=2), name='feats_norm')(feats) #may cannot be accept
    
    
    return feats_norm

def build_rmac_graph(rois, feature_maps, image_meta,
                         pool_size):
    """Builds the computation graph of the mask head of Feature Pyramid Network.
...
#                       name="pool_squeeze")(feats)
    ...
    # Addition
    rmac = KL.Lambda(lambda x: K.sum(x, axis=1), output_shape=(512,), name='rmac')(feats_norm)
    #[batch, channels].
    # # Normalization
    rmac_norm = KL.Lambda(lambda x: K.l2_normalize(x, axis=1), name='rmac_norm')(rmac)
    
    return rmac_norm, feats_norm

def build_given_roi_graph(rois, feature_maps, image_meta,
                         pool_size):
    """Builds the computation graph of the mask head of Feature Pyramid Network.
    ...
    # Shape: [batch, num_rois, MASK_POOL_SIZE, MASK_POOL_SIZE, channels]
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_given_roi")([rois, image_meta] + feature_maps)
    #[batch, num_boxes, pool_height, pool_width, channels].
#    feats = KL.Maximum(axis=(2, 3), name='rmac_maxpooling')(x)
    feat = KL.Lambda(lambda x: K.max(x, axis=(2, 3)))(x)
    # Normalization
    feat_norm = KL.Lambda(lambda x: K.l2_normalize(x, axis=2), name='given_roi_feats_norm')(feat) #may cannot be accept

    feat_norm = KL.Lambda(lambda x: x[:,0])(feat_norm)
    return feat_norm
...
#            print("data_generator image_id:", image_id)
...
#                print("yield inputs, outputs once!")
                # start a new batch
                b = 0
...
def TCM_moudle(pyramid_feature, channels=256, is_training=True, train_bn=False):
    """Builds the computation graph of Region Proposal Network.
    ...
    x1 = KL.Conv2D(channels, (3, 3), padding='same', activation='relu',
                   name='fpn_tcm_conv')(pyramid_feature)
    x1 = BatchNorm(name='bn_conv1')(x1, training=train_bn)
    x3 = KL.Conv2D(channels, (3, 3), padding='same', activation='relu',
                   name='fpn_tcm_conv')(pyramid_feature)
    x3 = BatchNorm(name='bn_conv1')(x3, training=train_bn)
    x2 = KL.Conv2D(channels, (3, 3), padding='same', activation='relu', 
                   name='fpn_tcm_conv')(x1)
    x2 = BatchNorm(name='bn_conv1')(x2, training=train_bn)
    global_mask = KL.Conv2D(1, (1, 1), padding='same', activation='sigmoid', 
                            name='fpn_tcm_conv')(x2)

    x3 = KL.Multiply()([global_mask, x3])
    
    x = KL.Add()([x1, x3])
    return [x, global_mask]


def build_TCM(pyramid_features):
    # build the text context module
    pyramid_features_tcm = []
    global_masks = []
    for pyramid_feature in pyramid_features:
        [p, global_mask]  = TCM_moudle(pyramid_feature)
        pyramid_features_tcm.append(p)
        global_masks.append(global_mask)
    return pyramid_features_tcm, global_masks
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir):
        ...
        """
        assert mode in ['training', 'inference', 'retrival']
        self.mode = mode
        self.config = config
...
#                    name="input_gt_masks", dtype=bool)
            ...
            input_given_roi = KL.Input(shape=[1, 4], name="input_given_roi")
            # Anchors in normalized coordinates
            input_anchors = KL.Input(shape=[None, 4], name="input_anchors")
...
#        rpn_feature_maps_tcm, global_masks = build_TCM(rpn_feature_maps)
...
#                    target_rois, input_gt_class_ids, gt_boxes, input_gt_masks])
            ...
            #generate rmac features  
            rmac_norm, feats_norm = build_rmac_graph(gt_boxes, mrcnn_feature_maps,
                                              input_image_meta,
                                              config.MASK_POOL_SIZE)
#            print(rmac_norm, feats_norm)
...
#                                              train_bn=config.TRAIN_BN)
            ...
            #generate rmac features  
            rmac, feats_norm = build_rmac_graph(detection_boxes, mrcnn_feature_maps,
                                              input_image_meta,
                                              config.MASK_POOL_SIZE)

            model = KM.Model([input_image, input_image_meta, input_anchors],
                             [detections, mrcnn_class, mrcnn_bbox,
                                 rpn_rois, rpn_class, rpn_bbox, rmac, feats_norm],
                             name='mask_rcnn')
        elif mode == 'retrival':
            # Network Heads
            # Proposal classifier and BBox regressor heads
            ...
            rmac, feats_norm = build_rmac_graph(detection_boxes, mrcnn_feature_maps,
                                              input_image_meta,
                                              config.MASK_POOL_SIZE)
            given_feat = build_given_roi_graph(input_given_roi, mrcnn_feature_maps,
                                              input_image_meta,
                                              config.MASK_POOL_SIZE)

            model = KM.Model([input_image, input_image_meta, input_anchors, input_given_roi],
                             [detections, mrcnn_class, mrcnn_bbox,
                                 rpn_rois, rpn_class, rpn_bbox, rmac, feats_norm, given_feat],
                             name='mask_rcnn')
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        layer_regex = {
            ...
            "4+": r"(res4.*)|(bn4.*)|(res5.*)|(bn5.*)|(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(rmac\_.*)",
            "5+": r"(res5.*)|(bn5.*)|(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(rmac\_.*)",
            # All layers
            "all": ".*",
        }
    ...
    def unmold_rmac(self, detections, original_image_shape,
                          image_shape, window, rmac, feats):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.
...
#            if full_masks else np.empty(original_image_shape[:2] + (0,))
        ...
        return boxes, class_ids, scores, rmac, feats

    def detect(self, images, verbose=0, given_rois=None):
        ...
        def resize_bbox(bbx, scale, padding, crop):
            ...
            return bbx1
        assert self.mode in  ["inference", "retrival"], "Create model in inference mode."
        assert len(images) == self.config.BATCH_SIZE, "len(images) must be equal to BATCH_SIZE"
        if self.mode == 'retrival':
            assert len(given_rois) == self.config.BATCH_SIZE, "len(given_rois) must be equal to BATCH_SIZE"
        if verbose:
        ...
        if self.mode == 'retrival':
            molded_given_rois = []
            for i, temp in enumerate(given_rois):
                molded_image, window, scale, padding, crop = utils.resize_image(
                images[i],
                min_dim=self.config.IMAGE_MIN_DIM,
                min_scale=self.config.IMAGE_MIN_SCALE,
                ...
                mode=self.config.IMAGE_RESIZE_MODE)
                boxes = []
                for given_roi in temp:
                    
                    given_roi =  resize_bbox(given_roi, scale, padding, crop)

                    image_shape = images[i].shape
                    box = [float(given_roi[0])/self.config.IMAGE_MIN_DIM, 
                            float(given_roi[1])/self.config.IMAGE_MIN_DIM, 
                            float(given_roi[2])/self.config.IMAGE_MIN_DIM, 
                            float(given_roi[3])/self.config.IMAGE_MIN_DIM]
                    boxes.append(box)
                molded_given_rois.append(boxes)
            molded_given_rois = np.array(molded_given_rois) 
        ...
        results = []                   #TODO  Aaron added
        if self.mode == 'inference':
            detections, _, _, _, _, _, rmac, feats_norm =\
                self.keras_model.predict([molded_images, image_metas, anchors, ], verbose=0)
                
            # Process detections
            
            for i, image in enumerate(images):
                final_rois, final_class_ids, final_scores, final_rmac, final_feats =\
                    self.unmold_rmac(detections[i],
                                           image.shape, molded_images[i].shape,
                                           windows[i], rmac[i], feats_norm[i])
                results.append({
                    "rois": final_rois,
        ...
        if self.mode == 'retrival':
            detections, _, _, _, _, _, rmac, feats_norm, given_bbox_feat =\
                self.keras_model.predict([molded_images, image_metas, anchors, molded_given_rois], verbose=0)
            ...
            for i, image in enumerate(images):
                final_rois, final_class_ids, final_scores, final_rmac, final_feats =\
                    self.unmold_rmac(detections[i],
                                           image.shape, molded_images[i].shape,
                                           windows[i], rmac[i], feats_norm[i])
                final_given_bbox_feat = given_bbox_feat[i]
                results.append({
                    "rois": final_rois,
                    "class_ids": final_class_ids,
                    "scores": final_scores,
                    "rmac": final_rmac,
                    'feats':final_feats,
                    'given_feat': final_given_bbox_feat
                })
